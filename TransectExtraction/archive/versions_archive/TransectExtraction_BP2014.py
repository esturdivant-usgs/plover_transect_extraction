...
############ Inputs #########################
arcpy.env.workspace=home= r"\\Mac\Home\Documents\ArcGIS\BreezyPoint2014.gdb"

year = '2014'
site = 'BP'

#points
dhPts = site+'_DHpts_'+year+'_edit1' 				# Dune crest
dlPts = site+'_DLpts_'+year+'_edit1' 				# Dune toe
fillValuesInPts = False
ShorelinePts = 'BP_SLPs_2014' 			          # Shoreline points
#SHL10 = 'BP_SHLPts_10m_2014'				   	  # beach slope from lidar within 10m of transect
#lines
MHW_oceanside = site+'_MHWocean_'+year+'_edit1'   # MHW, used to create BreezyBoundsPGon
inletLines = site+'_inletLines_'+year             # manually create lines based on the boundary polygon that correspond to end of land and cross the MHW line
armorLines = site+'_armorshoreward_'+year
extendedTransects = 'BreezyPt_extTransects_slim' # Created MANUALLY: see TransExtv4Notes.txt
#rawtransects = 'LongIsland_LT'     # National Assessment transects-done

#polygons
barrierBoundary = 'BreezyPt_'+year+'_Polygon'   # Barrier Boundary polygon; create with TE_createBoundaryPolygon.py
#grids
elevGrid = 'BP_DEM5m_2014'				# Elevation
...
############## Outputs ###############################
#points
dh2trans = site+'_DH2trans_'+year 							# DHigh within 10m
dl2trans = site+'_DL2trans_'+year						# DLow within 10m
arm2trans = site+'_arm2trans_'+year
shl2trans = site+'_SHL2trans_'+year							# beach slope from lidar within 10m of transect
MLWpts = site+'_MLW2trans_'+year                       # MLW points calculated during Beach Width calculation
#lines
shoreline = site+'_ShoreBetweenInlets_'+year        # Complete shoreline ready to become route in Pt. 2
#grids
slopeGrid = site+'_slope5m_'+year
#transects
transects_final = site+year+'populatedTransects'
...
extendedTransects = 'BreezyPt_extTransects_slim'
...
def AddXYAttributes(fc,newfc,prefix):
    ...
    nad83 = u"GEOGCS['GCS_North_American_1983',DATUM['D_North_American_1983',SPHEROID['GRS_1980',6378137.0,298.257222101]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]];-400 -400 1000000000;-100000 10000;-100000 10000;8.98315284119521E-09;0.001;0.001;IsHighPrecision"
    arcpy.Project_management(fc,newfc,nad83)
...
def PointsToTransOrder(fc,oldPts,newPts,fieldnamesdict,tempfile='trans_temp',tolerance='25 METERS'):
    # Create temp file of TransOrder with point metrics
    # 1. Create ID field and populate with OBJECTID
    # 2. Join nearest point to transect --> tempfile
    ReplaceFields(oldPts,{'ID':'OID@'},'SINGLE')
    arcpy.SpatialJoin_analysis(fc,oldPts, tempfile,'#','#','#',"CLOSEST",tolerance) # one-to-one # Error could result from different coordinate systems?
    if newPts:
...
def BeachPointMetricsToTransects(transects,oldPts,newPts,fieldnamesdict,firsttime=True, tempfile='trans_temp', tolerance='25 METERS'):
    ...
    if firsttime:
        PointsToTransOrder(transects,oldPts,newPts,fieldnamesdict,tempfile)
    else:
...
if arcpy.Exists(elevGrid):
    ...
    arcpy.sa.ExtractMultiValuesToPoints(arm2trans+'_withZ',elevGrid)
...
armorfields = {'Arm_Lon':'Arm_Lon','Arm_Lat':'Arm_Lat','Arm_easting':'Arm_easting','Arm_northing':'Arm_northing','Arm_z':'Arm_z'}
arcpy.JoinField_management(extendedTransects,"OBJECTID",arm2trans,'FID_'+extendedTransects,armorfields.keys())

# Dune metrics
dhfields = {'DH_Lon':'lon_sm', 'DH_Lat':'lat_sm', 'DH_easting':'east_sm', 'DH_northing':'north_sm', 'DH_z':'dhigh_z'}
BeachPointMetricsToTransects(extendedTransects,dhPts,dh2trans,dhfields, True, pt2trans_disttolerance)
dlfields = {'DL_Lon':'lon_sm', 'DL_Lat':'lat_sm', 'DL_easting':'east_sm', 'DL_northing':'north_sm', 'DL_z':'dlow_z'}
BeachPointMetricsToTransects(extendedTransects,dlPts,dl2trans,dlfields, True, pt2trans_disttolerance)
...
Requires: clipped transects, dh2trans, dl2trans, elevation, slope, recharge, Habitat
'''
...
with arcpy.da.UpdateCursor(tranSplitPts, ('DistSegDH','DistSegDL','Dist_Seg','DistDH','DistDL')) as cursor:
    for row in cursor:
        try:
            row[0] = row[2]-row[3]
        except:
...
ReplaceValueInFC(transSplitPts_final,[], None, -99999)
