...
trans_df.to_pickle(os.path.join(out_dir, extTrans_fill+'.pkl'))
...
finalmessage = "The final products ({}) were exported as a CSV to {}."\
...
extT_fill
'''
# Save final transects with fill values as FC, CSV, and raster
# First, check for missing fields
missing_Tfields = fieldsAbsent(extendedTransects, transect_fields)
if missing_Tfields:
    print('WARNING: Required fields missing from {}:\n{}'.format(missing_Tfields))
# Save as FC with fill
...
# Export from gdb to out_dir
arcpy.CopyRaster_management(out_rst, os.path.join(out_dir, rst_trans_grid))

# Join required fields from extendedTransects that transPts are missing
...
# Save pts as feature class with Nulls
...
# Save as SHP
arcpy.FeatureClassToFeatureClass_conversion(transPts_fill, out_dir, transPts_shp+'.shp')
# Save as CSV
