...
def join_df_2raster(df, rst_ID, out_rst, IDfield='sort_ID'):
    # Join fields from in_tbl to rst_ID to create new out_rst
    trans_arr = df.to_records()
    trans_tbl = os.path.join(arcpy.env.workspace, 'table_temp')
    arcpy.da.NumPyArrayToTable(trans_arr, trans_tbl)
    RemoveLayerFromMXD('rst_lyr') # in case of reprocessing
    arcpy.MakeTableView_management(trans_arr, 'tableview')
    arcpy.MakeRasterLayer_management(rst_ID, 'rst_lyr')
    arcpy.AddJoin_management('rst_lyr', 'Value', 'tableview', IDfield)
    arcpy.CopyRaster_management('rst_lyr', out_rst)
    return out_rst
...
def calc_beach_width(pts_df, maxDH=2.5, tID_fld='sort_ID'):
    ...
    for tID, tran in trans_df.iterrows():
        ...
        if np.isnan(tran.DL_x) and tran.DH_z <= maxDH:
            iDL = [tran['DH_x'], tran['DH_y'], tran['DH_zMHW']]
        elif np.isnan(tran.DL_x) and not np.isnan(tran.Arm_x):
            iDL = [tran['Arm_x'], tran['Arm_y'], tran['Arm_z']]
        else:
            iDL = [tran['DL_y'], tran['DL_x'], tran['DL_zMHW']]
        # get transPt nearest to DL (ptDLy, ptDLx)
        ...
        pt2dldist = np.hypot(Xtran - iDL[0], Ytran - iDL[1])
        if np.isnan(pt2dldist.idxmin()):
            ptDLy = ptDLx = np.nan
        else:
            ptDLy = Ytran[pt2dldist.idxmin()] #FIXME: Ben's code takes the first point along the transect if there is no DL equivalent
            ptDLx = Xtran[pt2dldist.idxmin()]
        # Compare position of armoring to position of DL equivalent
        if not np.isnan(tran.Arm_x) and not np.isnan(ptDLx): # if isnan(Ae) == 0 & isnan(DLe) == 0,
            d1 = np.hypot(tran.SL_x-ptDLx, tran.SL_y-ptDLy)
            d2 = np.hypot(tran.SL_x-tran.Arm_x, tran.SL_y-tran.Arm_y)
            if d1 < d2:
                Xout = ptDLx
                Yout = ptDLy
                Zout = iDL[2]
                ...
                Yout = tran.Arm_y
                Zout = tran.Arm_zMHW
        elif not np.isnan(tran.Arm_x) and np.isnan(ptDLx): # elseif isnan(Ae) == 0 & isnan(DLe) == 1,
            Xout = tran.Arm_x
            Yout = tran.Arm_y
            Zout = tran.Arm_zMHW
        elif not np.isnan(ptDLx) and np.isnan(tran.Arm_x): # elseif isnan(DLe) == 0 & isnan(Ae) == 1,
            Xout = ptDLx
            Yout = ptDLy
            Zout = iDL[2]
        else:
            ...
            Zout = np.nan
        # Get beach width
        uBW[tID] = np.hypot(tran.SL_x - Xout, tran.SL_y - Yout) # problem: Xout
        uBH[tID] = Zout # Use elevation from transPt instead of from dune point?

    # Add new uBW and uBH fields to trans_df
    beachWH = pd.concat([uBW, uBH], axis=1)
    pts_df_updated = pts_df.join(beachWH, on=tID_fld, how='outer')
    return(pts_df_updated, beachWH)
