...
ShorelinePts = 'SLPs{site}{year}'.format(**SYvars)
dhPts = 'BP_DH_2010_edit2_orig' #'FI{year}_DHpts'.format(**SYvars)				# Dune crest
dlPts = 'BP_DL_2010_edit2_orig' #'FI{year}_DLpts'.format(**SYvars) 		  # Dune toe
inletLines = 'BP_inletLines' #.format(**SYvars)  # manually created
armorLines = 'BP_armorshoreward_2010' #.format(**SYvars)
barrierBoundary = 'BreezyPoint2010Polygon' #FI{year}_BNDpoly_MHWocean'.format(**SYvars)   # Barrier Boundary polygon
shoreline = 'BP{year}_ShoreBetweenInlets'.format(**SYvars)    # Complete shoreline ready to become route in Pt. 2
elevGrid = 'BP{year}_DEM'.format(**SYvars)				# Elevation
elevGrid_5m = elevGrid+'_5m'				# Elevation
#habitat = 'habitat_201211' 			# Habitat
slopeGrid = 'BP_slope5m_2010'.format(**SYvars)
...
rst_transID = "{site}{year}_rstTransID".format(**SYvars)
