...
ShorelinePts = 'BP{year}_SLpts'.format(**SYvars)
dhPts = 'BP_DH_2012_edit2' #'FI{year}_DHpts'.format(**SYvars)				# Dune crest
dlPts = 'BP_DL_2012_edit2' #'FI{year}_DLpts'.format(**SYvars) 		  # Dune toe
inletLines = 'BP_inletLines' #.format(**SYvars)  # manually created
armorLines = 'BP_armorshoreward_{year}'.format(**SYvars)
barrierBoundary = 'BreezyPt_{year}_Polygon'.format(**SYvars) # Barrier Boundary polygon
shoreline = 'BP{year}_ShoreBetweenInlets'.format(**SYvars)    # Complete shoreline ready to become route in Pt. 2
elevGrid = 'BreezyPoint_USACE{year}_DEM'.format(**SYvars)				# Elevation
elevGrid_5m = elevGrid+'_5m'				# Elevation
#habitat = 'habitat_201211' 			# Habitat
slopeGrid = 'BP_slope5m_{year}'.format(**SYvars)
