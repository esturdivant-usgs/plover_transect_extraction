...
from TE_config_Monomoy2014 import *
...
arcpy.Sort_management(extendedTransects,transwork,transUIDfield)
...
BeachPointMetricsToTransects(extendedTransects, dhPts, dh2trans, dhfields, joinfields=[transUIDfield,transUIDfield], firsttime=False, tempfile=tempfile, tolerance=pt2trans_disttolerance)
...
arcpy.PointsToLine_management("verts_temp","trans_temp",transUIDfield) # creates trans_temp: clipped transects with single vertices
...
with arcpy.da.UpdateCursor(baseName,'*') as cursor:
    for row in cursor:
        ...
        if cp: # if closest point was found calculate beach width with that point, otherwise skip
            ...
            row[cursor.fields.index('beachWidth_MHW')] = beachWidth_MHW
...
with arcpy.da.UpdateCursor(transPts_presort, ['Dist_Seg','Dist_MHWbay','seg_x','seg_y','SL_easting','SL_northing','WidthPart']) as cursor:
    for row in cursor:
        ...
        cursor.updateRow(row)
"""
Replaced by Sort on transUIDfield and Dist_Seg below
# Create unique id SplitSort by sorting on transUIDfield and DistSeg
arcpy.AddField_management(transPts_presort,"id_temp","TEXT")
#Create temp file with points sorted by [transUIDfield]_[Dist_Seg] # Calc field - 24 seconds; cursor - 4.5 seconds
with arcpy.da.UpdateCursor(transPts_presort, (transUIDfield,'Dist_Seg','id_temp')) as cursor:
    for row in cursor:
        try:
            dist = str(int(row[1]))
        except:
