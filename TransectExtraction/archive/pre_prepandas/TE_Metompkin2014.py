...
year = '2014'
site = 'Metompkin'
SiteYear_strings = {'site': 'Metompkin',
                    'year': '2014',
                    'region': 'Delmarva'}
...
if arcpy.Exists(pts_elevslope):
    ...
    with arcpy.UpdateCursor(tranSplitPts,['PointZ','PointZ_mhw']) as cursor:
        for row in cursor:
            row[1] = row[0] - MHW
            cursor.updateRow(row)
...
#arcpy.MakeTableView_management(tranSplitPts_fill,tranSplitPts_fill)

finalmessage = "The final points ("+tranSplitPts_shp+") were exported as a shapefile to "+out_dir+". \n" \
      "\nNow enter the USER to save the table: \n\n" \
      "1. Export the new table ("+tranSplitPts_fill+") as Text with a '.csv' extension. \n" \
